...
export async function trackDownload(componentName: string) {
  ...
  try {
    const response = await fetch("https://workers.joyco.studio/registry/downloads", {
      method: "POST",
      headers: {
    ...
    console.error(`[Registry Download] Failed to track for ${componentName}:`, error.message)
    return
  }
}
