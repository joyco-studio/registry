...
The component uses the **Web Animations API (WAAPI)** with `steps(1)` easing to create discrete frame jumps. This provides:

- Native browser animation performance
- Dynamic `frameDuration` changes without resetting playback position
- Smooth play/pause transitions
- Reduced motion preference support (shows static first frame)
