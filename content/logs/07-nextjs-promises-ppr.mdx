...
flowchart TD
    ...
    A --> D["Render HTML shell immediately\n(with Suspense fallbacks)"]

    B --> E["Stream resolved data\nas promises settle"]
    C --> E
    D --> E
...
block-beta
    ...
    style nav fill:#bfdbfe,stroke:#2563eb
    style main fill:#bfdbfe,stroke:#2563eb
    style skeleton1 fill:#fef9c3,stroke:#ca8a04
    style skeleton2 fill:#fef9c3,stroke:#ca8a04
...
```mermaid
flowchart TD
    A(["Static shell served — 0ms"]) --> B["getFlags resolves — 80ms"]
    A --> C["getCart resolves — 120ms"]

    B --> D["CheckoutButton renders\nCheckout with Express Pay"]
    C --> E["CartCount: 3\nCartTotal: $149.97"]
...
```mermaid
gantt
    title Rendering Timeline — Promise Forwarding + PPR
    dateFormat X
    axisFormat %Lms

    section Static
    Shell served from CDN           :done, 0, 10

    section Flags
    getFlags resolves               :active, 0, 80
    CheckoutButton + BetaBanner     :milestone, 80, 0

    section Cart
    getCart resolves                 :active, 0, 120
    CartCount + CartTotal           :milestone, 120, 0

    section Result
    Fully interactive               :crit, 120, 150
```
...
flowchart TD
    ...
    style request fill:#f0fdf4,stroke:#16a34a
    style B3 fill:#bfdbfe,stroke:#2563eb
    style R2 fill:#bbf7d0,stroke:#16a34a
    style R7 fill:#e9d5ff,stroke:#7c3aed
    style R8 fill:#e9d5ff,stroke:#7c3aed
```
...
|---------|------------|------------|
| Promise forwarding to client | ✓ Works | ✓ Works |
| `use()` hook | ✓ React 19 | ✓ React 19 |
