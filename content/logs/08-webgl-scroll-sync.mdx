...
Firefox's APZ (Async Pan/Zoom) documentation gives the most precise breakdown:
...
Under ideal conditions, JS-driven effects are **1 frame behind** compositor scroll. Under main-thread load (GC pauses, layout thrashing, heavy React renders), 2-3 frames. At 60fps, that's 16-48ms of positional drift.
...
- **`position: fixed` children**: Elements inside the translated container can't be `position: fixed` relative to the viewport. A CSS `transform` on an ancestor creates a new containing block, so fixed children become fixed to the container instead. This breaks sticky headers, modals, tooltips -- anything that needs to stay in the viewport.
...
```mermaid
flowchart TD
    subgraph fixed ["Fixed approach drift"]
        direction TB
        F1["Canvas stays in window"] --> F2["Content rendered with stale scrollY"]
        F2 --> F3["WebGL images separate<br/>from DOM placeholders"]
    end

    subgraph absolute ["Absolute approach drift"]
        direction TB
        A1["Canvas moves with page"] --> A2["Content stays aligned with DOM"]
        A2 --> A3["Canvas edges slightly miss<br/>viewport bounds"]
    end
...
This is the main tradeoff of the absolute approach. Because the canvas has a CSS `transform` applied to it, any `position: fixed` descendants stop being fixed to the viewport. Per the CSS spec, a `transform` on an ancestor creates a new containing block for fixed-position descendants.

This means if your page design requires elements that are:
- Inside or overlapping the canvas container
- And `position: fixed` to the viewport (sticky nav, floating UI, modals)

...they will scroll with the canvas instead of staying put. You need to either:
- Structure your DOM so fixed elements are **outside** the canvas container (siblings, not children)
- Or use a separate layer/portal for fixed UI that doesn't live inside the transformed element

### The vertex shader: page coords to canvas coords

In this approach, each WebGL mesh knows its **absolute page position** (read from `getBoundingClientRect()` + `scrollY` on resize). The vertex shader converts that to canvas-local pixel coordinates by subtracting the scroll offset:

```glsl
uniform vec2 u_resolution;    // Canvas size in pixels
uniform vec2 u_scrollOffset;  // scrollY - padding offset
uniform vec2 u_domXY;         // Absolute page position of DOM element
uniform vec2 u_domWH;         // Width/height in pixels

void main() {
    // Absolute page coords → canvas-local pixel coords
    vec2 pixelXY = u_domXY - u_scrollOffset + u_domWH * 0.5;

    // Flip Y (DOM Y goes down, GL Y goes up)
    pixelXY.y = u_resolution.y - pixelXY.y;

    // Scale unit quad to DOM element size
    pixelXY += position.xy * u_domWH;

    // Pixel coords → NDC (-1 to +1)
    gl_Position = vec4(pixelXY / u_resolution * 2.0 - 1.0, 0.0, 1.0);
}
```

Because an identity `Camera()` is used (no projection/view matrix), the vertex shader output goes directly to clip space. This is a 2D orthographic projection done entirely in the shader.

### When to use it

When you have a real webpage with DOM content -- text, navigation, interactive elements -- and the WebGL canvas is an enhancement layer (background effects, image treatments, transitions). Native scroll stays intact. Browser features work. Accessibility works. The only constraint is managing `position: fixed` elements around the transformed canvas.
...
flowchart TD
    ...
    ABS_HOW --> ABS_CON["Edge clipping (fix with padding)<br/>position: fixed children break"]
...
| **`position: fixed`** | Works (no transform on ancestors) | Breaks inside canvas container |
